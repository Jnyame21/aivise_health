...
export const isWithinWorkingHours = (selectedTime: string): boolean=> {
...
export const hasTimePassed = (selectedTime: string): boolean=> {
  const now = new Date();
  const [inputHours, inputMinutes] = selectedTime.split(':').map(Number);
  const selectedDate = new Date();
  selectedDate.setHours(inputHours, inputMinutes, 0, 0);
